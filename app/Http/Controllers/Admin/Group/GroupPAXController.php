...
class GroupPAXController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $price_offer = PriceOffer::where('group_req_id', $request['id'])->first();
        $preUpload   = GroupPAXInfo::where('group_id', $request['id'])->count();
        $group_req   = GroupRequest::find($price_offer->group_req_id);
        // dd($group_req);
        $group_pnr = Group_pnr::where('id', $group_req->group_pnr_id)->first();
        if ($group_pnr->max_refund_percent == 'percent') {
            $max_refund = floor(($group_pnr->total_seat * $group_pnr->max_refund) / 100);
        } else {
            $max_refund = $group_pnr->max_refund;
        }
        ...
        if (empty($request->pax) || ! is_array($request->pax)) {
            return response()->json(['message' => 'No PAX data provided'], 422);
        }

        $remaining = $price_offer->total_traveler - $preUpload;

        if (count($request->pax) > $remaining) {
            return response()->json([
                'message'          => 'Total PAX exceeds the offered total traveler count',
                'total_traveler'   => $price_offer->total_traveler,
                'already_uploaded' => $preUpload,
                'remaining_slots'  => max(0, $remaining),
                'trying_to_upload' => count($request->pax),
            ], 400);
        }
        ...
        foreach ($request->pax as $key => $value) {
            $passportNo = $value['passport_no'] ?? null;

            if (! $passportNo) {
                return response()->json(['message' => 'Passport number is required at row ' . ($key + 1)], 422);
            }

            if (in_array($passportNo, $seenInRequest)) {
                $duplicates[] = "Row " . ($key + 1) . ": passport {$passportNo} is duplicated within the uploaded file";
            } else {
                $seenInRequest[] = $passportNo;
            }

            $existsInGroup = GroupPAXInfo::where('group_id', $request['id'])
                ->where('passport_no', $passportNo)
                ->exists();

            if ($existsInGroup) {
                $duplicates[] = "Passport {$passportNo} is already uploaded for this group";
            }
        }

        if (! empty($duplicates)) {
            return response()->json([
                'message'    => 'Duplicate passenger(s) found. Nothing was saved.',
        ...
        try {
            foreach ($request->pax as $key => $value) {
                ...
                $groupPAXInfo->gender      = $value['gender'] ?? null;
                $groupPAXInfo->dob         = $value['dob'] ? date('Y-m-d', strtotime($value['dob'])) : null;
                $groupPAXInfo->created_by  = auth()->id();
                $groupPAXInfo->save();

                $lastSavedPax = $groupPAXInfo;

                $existingTraveller = DB::table('travellers')
                    ->where('passport_number', $value['passport_no'])
                    ->first();
                ...
                if (! $existingTraveller) {
                    DB::table('travellers')->insert([
                        ...
                        'first_name'           => $value['first_name'],
                        'last_name'            => $value['last_name'],
                        'full_name'            => trim(($value['title'] ?? '') . ' ' . $value['first_name'] . ' ' . $value['last_name']),
                        'agent_id'             => $agent->id,
                        'passport_number'      => $value['passport_no'],
                        'passport_expiry_date' => $value['expiry_date'] ? date('Y-m-d', strtotime($value['expiry_date'])) : null,
                        'email'                => $value['email'] ?? null,
            ...
            $finalPaxCount = GroupPAXInfo::where('group_id', $request['id'])->count();

            if ($finalPaxCount >= $price_offer->total_traveler) {
                $price_offer->status = 'PAX Uploaded';
            } elseif ($finalPaxCount >= ($price_offer->total_traveler - $max_refund)) {
                $price_offer->status = 'PAX Partially Uploaded';
            } else {
                $price_offer->status = 'PAX Partially Uploaded';
            }
            ...
            DB::rollBack();
            return response()->json(['message' => 'Failed to save PAX info', 'error' => $e->getMessage()], 500);
        }

        return response()->json(['message' => 'Group PAX info stored successfully', 'data' => $lastSavedPax], 201);
    }
