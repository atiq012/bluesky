...
// This component is intentionally not registered in the router. It is a visual
// replacement candidate for create.vue and can be wired in when approved.
const form = reactive({
...
//         filteredTraveler.value = [];
//         return;
//     }
//     try {
//         const response = await axiosInstance.post('get-travelers-data-by-search', { parm: param });
//         if (response.data.length > 0) {
//             showTravelerList.value = true;
//             filteredTraveler.value = response.data.map(item => ({
//                 id: item.id,
//                 text: item.full_name || `${item.first_name} ${item.last_name}`,
...
//         }
//     } catch (err) {
//         console.error('Error searching traveler:', err);
//     }
// }
...
//         // Auto-fill travelerCreation form fields
//         form.title = data.title || '';
//         form.firstName = data.first_name || '';
//         form.lastName = data.last_name || '';
//         form.dob = data.dob ? moment(data.dob).format('DD-MMM-YYYY') : '';
//         form.gender = data.gender || '';
//         form.nationality = data.nationality || '';
//         form.passportNo = data.passport_number || '';
//         form.passportExpiry = data.expiry_date ? moment(data.expiry_date).format('YYYY-MM-DD') : '';
//         form.email = data.email || '';
//         form.phone = data.phone || '';
//         if (data.passport_path) {
//             form.passportFiles = [{ url: data.passport_path }];
//         }
//     } catch (error) {
//         console.error('Error loading traveler details:', error);
//     }
// }

// Select traveler and auto-fill travelerCreation form
async function selectTraveler(traveler) {
    isSelecting.value = true; // Mark as selecting
    showTravelerList.value = false;
    searchParam.value = traveler.full_name || traveler.first_name || '';
    try {
        ...
        form.passportExpiry = d.passport_expiry_date ? moment(d.passport_expiry_date).format('YYYY-MM-DD') : '';
        form.email = d.email || '';
        form.phone = d.phone || '';
        ...
        if (d.passport_path) {
            form.passportFiles = [{
                ...
                file: { size: d.passport_file_size || 0 }
            }];
        }
        ...
        setTimeout(() => { isSelecting.value = false; }, 200);
    }
}
...
// Watch search query and fetch results from API
watch(searchParam, async (newVal) => {
    // Skip searching if update came from selecting an item
    if (isSelecting.value) return;
    if (!newVal?.trim()) {
        showTravelerList.value = false;
        filteredTravelers.value = [];
        return;
    }
    try {
        const response = await axiosInstance.post('get-travelers-data-by-search', { parm: newVal });
        if (response.data && response.data.length > 0) {
            filteredTravelers.value = response.data;
            showTravelerList.value = true;
        } else {
            filteredTravelers.value = [];
            showTravelerList.value = false;
...
function validate() {
    ...
    // Title Validation based on Pax Type
    if (!form.title) {
    ...
    if (!isChildOrInfant.value) {
        ...
        // Optional for Child/Infant: validate format only if user typed something
...
async function save() {
    ...
    try {
        ...
        const response = await axios.post('/api/traveler/data/save', payload, {
            headers: { Accept: 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}) },
        });

        const message =
            response.data.message || 'Traveller created successfully.';
                ...
                <h2 class="accordion-header">
                    <div class="accordion-button traveler-header-btn">
                        ...
                        <span class="traveler-header-text">
                            <span class="traveler-header-index">New Traveller</span>
                            <span class="traveler-header-type">Create traveller profile</span>
                        </span>
                    </div>
                </h2>
                ...
                <div class="traveller-accordion-body">
                    <form @submit.prevent>
                        ...
                        <div class="mt-4 text-end">
                            ...
                            <AppButton variant="save" label="Save" size="md" :loading="isSubmitting" @click="save" />
                        </div>
                    </form>
                </div>
            </div>
                ...
                <div class="modal-body-section">
                    ...
                    <div class="modal-cta-row">
                        <button type="button" class="btn-register-another" @click="registerAnother">
                            <i class="fa fa-plus-circle"></i> Create Another
                        </button>
                        <button type="button" class="btn-home" @click="goToTravelerList">
